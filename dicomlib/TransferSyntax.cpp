	...
	*/
	...
	bool TS::hasCompiledPixelCodec() const
	{
		...
		if(uid_ == JPEG_EXTENDED_PROCESS_2_4_TRANSFER_SYNTAX ||
			uid_ == JPEG_EXTENDED_PROCESS_3_5_TRANSFER_SYNTAX ||
			uid_ == JPEG_SPECTRAL_SELECTION_PROCESS_6_8_TRANSFER_SYNTAX ||
			uid_ == JPEG_FULL_PROGRESSION_PROCESS_10_12_TRANSFER_SYNTAX ||
		...
		if(uid_ == JPEG_XL_LOSSLESS_TRANSFER_SYNTAX ||
			uid_ == JPEG_XL_JPEG_RECOMPRESSION_TRANSFER_SYNTAX ||
			uid_ == JPEG_XL_TRANSFER_SYNTAX)
			return true;
#endif
		return false;
	}
