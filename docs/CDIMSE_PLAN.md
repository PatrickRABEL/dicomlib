...
| Network end-to-end tests | Current tests cover command set construction, selected dispatch/SCU code paths, local P-DATA round trips, a local A-ASSOCIATE primitive exchange with C-ECHO, and thread-backed C-ECHO. Full thread-backed `Server`/`ClientConnection` association tests for C-STORE, C-FIND, C-GET, C-MOVE, and C-CANCEL are still required. |
...
1. Add thread-backed `Server`/`ClientConnection` association tests for C-STORE,
   C-FIND, C-GET, C-MOVE, and C-CANCEL.
